...
### Changed
- **Gazebo mission harness:** fork `tb3_simulation_launch.py` so gz starts after xacro,
  bridge/spawn/Nav2 follow gz; `sim_mission.py` coerces float params and waits for AMCL +
  `bt_navigator` lifecycle active; mission node delayed 45s in `tb3_gazebo_mission.launch.py`.
  See [docs/simulation.md](docs/simulation.md) §10.5.
