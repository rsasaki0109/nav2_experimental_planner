...
**v0.6.0** — **初の学習済みモデルがループに入る**リリース。これまで「Learned models propose. Classical safety disposes. Nav2 executes.」の "learned" は解析的プレースホルダだけが担っていた（学習済みモデルは未稼働）。本リリースで初の curated 学習済みモデル 2 種を、いずれも**実際の C++ ONNX 推論経路で end-to-end**（unit-test fixture ではなく）収録: costmap 条件付き flow の **path モデル**（Mode B global planner）と **trajectory モデル**（Mode A local controller）。両者とも costmap を読んで全提案を空き側へ寄せ、決定論的層が検証する。[model_zoo](model_zoo)（manifest / model card / 再現スクリプト付き）を実体化し、比較ベンチ 2 種が learned エントリを走らせ、**learned Mode A は open シナリオで閉ループ goal 到達**。小型合成モデルの限界も正直に明示（アーキテクチャは end-to-end で動く／伸びしろはモデル側）。基盤は v0.5.0（ND controller + 比較ベンチ 2 種 + 選択ガイド + getting_started 導線）、v0.4.0（**8 種の classical GlobalPlanner** + 初の classical Controller VFH+）、v0.3.0 までの生成系（決定論的安全層 + ONNX + flow/diffusion/consistency + costmap 条件付け + 学習パイプライン + RViz/Foxglove 可視化）。変更履歴は [CHANGELOG.md](CHANGELOG.md)、今後の計画は [docs/roadmap.md](docs/roadmap.md)。API は 1.0.0 まで安定化されていません。
