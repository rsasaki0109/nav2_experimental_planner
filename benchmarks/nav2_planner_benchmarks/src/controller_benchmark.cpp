...
int main(int argc, char ** argv)
{
  ...
  std::cout << "All are `nav2_core::Controller` plugins absent from upstream Nav2 "
    ...
    "4.27 m traverse where the other learned models stall at ~1.0 m), and drives "
    "markedly further and closer on the *frontal obstacle* (1.69 m / 0.20 m clearance "
    "vs ~1.0 m / 0.75 m), though it still times out on the dead-ahead *frontal* block "
    "and the *corridor* — the dodge oracle does not centre in a passage, and a "
    "dead-ahead inflated block needs a sharper commit than it learned. (In the DAgger "
    "costmap sim the two fixes lift closed-loop threading from 1/4 to 4/4; the gap to "
    "this real benchmark is the inflated dead-ahead block and the corridor.) "
    "**Diffusion (Mode A, "
