...
int main(int argc, char ** argv)
{
  ...
  std::cout << "# Local controller comparison (VFH+ / ND / learned & hybrid Mode A)\n\n";
  ...
  std::cout << "All are `nav2_core::Controller` plugins absent from upstream Nav2 "
    ...
    "it safely short of the block. **Diffusion (Mode A, hybrid)** adds a classical "
  ...
  std::cout << "_Min clearance is the smallest robot-to-obstacle distance over the "
    ...
    "collision). The **hybrid** adds a classical fallback (VFH+) that takes over when "
