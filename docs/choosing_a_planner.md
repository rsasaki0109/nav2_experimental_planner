...
`nav2_experimental_planner` は Nav2 公式に無い planner / controller を多数収録している。本ガイドはその中から**状況に応じてどれを選ぶか**の道標。実測の比較は [planner_comparison.md](planner_comparison.md)（GlobalPlanner 8 種）と [controller_comparison.md](controller_comparison.md)（reactive Controller 2 種）を参照。
